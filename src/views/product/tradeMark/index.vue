      ...
      <el-table-column label="操作">
        <template slot-scope="{ row, $index }">
          <el-button
            ...
            >修改</el-button
          >
          <el-button type="danger" icon="el-icon-edit" size="mini" @click="deleteTradeMark(row)"
            >删除</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    // 自定义校验规则 不放在return中
    // value用户输入的文本 rule规则 callback回调函数
    var validateTmName = (rule, value, callback) => {
      if (value.length < 2 || value.length > 10) {
        callback(new Error("名称在 2 到 10 个字符"));
      } else {
        callback();
      }
    };
    return {
      ...
      rules: {
        tmName: [
          ...
          // message 提示信息
          // trigger:用户行为设置（事件的设置:blur失焦、change文本改变）
          { required: true, message: "请输入品牌名称", trigger: "blur" },
          { validator: validateTmName, trigger: "change" }
        ],
        logoUrl: [
          // dialog不属于表单元素 用trigger没用
