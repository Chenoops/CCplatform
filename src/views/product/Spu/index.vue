<template>
  <div>
      <el-card>
        ...
        <!-- show在categoryselect组件中控制disabled 这里是父传子 -->
      ...
      <el-card>
        ...
        <!-- 展示SPU列表的结构 -->
        <div v-show="scene==0">
          <el-button type="primary" icon="el-icon-plus" :disabled="!category3Id" @click="addSku(row)">添加SPU</el-button>
        ...
        <sku-form v-show="scene==2" @changeScenes="changeScenes" ref="sku"></sku-form>
      </el-card>
...
methods: {
  ...
  //获取SPU列表数据的方法 + 点击分页器获取新一页数据
  async getSpuList(pages=1) { //默认参数pages 不能用==
    this.page=pages;
  ...
  // 自定义事件回调（SpuForm）传过来的是一个对象
  changeScene({scene,flag}) {
    //flag这个形参为了区分保存按钮是添加还是修改
    // 切换状态(场景)
    this.scene=scene
    // 子组件通知父组件切换场景 需要再次获取SPU的列表进行展示 停留在当前页
    if(flag=='添加') {
      // 添加则回到第一页
      this.getSpuList()
    } else {
  ...
  close(done) {
    ...
    //清除sku列表的数据
    this.skuList=[]
    // done用于关闭对话框 elementui文档的before-close属性介绍
    done()
  }
