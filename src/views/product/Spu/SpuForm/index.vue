<template>
  <div>
    ...
    <el-form ref="form" label-width="80px" :model="spu">
      ...
      <el-form-item>
        <!-- 上传图片：action图片上传的地址 要加上/dev-api 否则会有跨域问题
        list-type: 文件列表的类型 on-preview:图片预览的时候会出发  on-remove:当删除图片的时候会出发 
      ...
      <el-form-item label="销售属性">
        <el-select :placeholder="`还有${unSelectSaleAttr.length}个属性未选择`" value="" v-model="attrIdAndAttrName">
...
export default {
  ...
  data() {
    return {
      dialogImageUrl: "",
      dialogVisible: false,
      //修改SPU，spu属性初始化的时候是一个空的对象,在修改SPU的时候，会想服务器发请求，返回SPU信息（对象），在修改的时候可以利用服务器返回的这个对象收集最新的数据提交给服务器
      //添加SPU，如果是添加SPU的时候并没有向服务器发请求，数据收集到哪里呀[SPU]，收集数据的时候有哪些字段呀，看文档传参内容 saveinfo
      spu: {
        //三级分类的id
        category3Id: 0,
        //描述
        ...
        // 品牌的id
        tmId: 0,
        //收集SPU图片的信息 照片墙的数据没有存放在spu中 点击保存时整理参数后才会传进来
        spuImageList: [
          {
            // id: 0,
            // imgName: "string",
  ...
  methods: {
    ...
    // 初始化SpuForm数据 (SPU中点击修改时)
    async initSpuData(spu) {
      // spu就是传进来的row 包括category3Id与id
      // console.log(spu);
    ...
    addSaleAttr() {
      ...
      //向SPU对象的spuSaleAttrList属性里面添加新的销售属性 属性值spuSaleAttrValueList暂时没有 设置为[]
    ...
    handleInputConfirm(row) {
      ...
      // 属性值不能重复 这里也可以用some 为什么这里==不行 只能用!=  ？？？？？？？？？？？？？
      ...
      // 新增销售属性值 根据文档中的要求传递字段 前一个属性kv一致省略v
    ...
    async addOrUpdateSpu() {
      //整理参数：需要整理照片墙的数据
      //携带参数：对于图片，需要携带imageName与imageUrl字段 已有的图片有这两个字段 自己新增的图片只有自己的name和url(且是本地地址) 用map
      ...
      if(result.code==200) {
        // 提示
        this.$message({type: 'success',message:'spu保存成功'})
        // 通知父组件回到场景0
    ...
    cancel() {
      ...
      //组件实例this._data,可以操作data当中响应式数据 有响应式数据就有this._data
